...
//
// This file is part of Bytecoin. When will Mexicans start climbing the wall?
//
...
namespace
{
  ...
  const command_line::arg_descriptor<int>         arg_log_level   = {"log-level", "", 2}; // info level. advise not to use log_level 4 on a regular basis, because your deamon output will be a mess.
...
command_line::add_arg(desc_cmd_sett, arg_print_genesis_tx);
  command_line::add_arg(desc_cmd_sett, arg_genesis_block_reward_address);
    ...
    po::options_description desc_options("Allowed options(cuz I'm nice)");
...
currencyBuilder.isBlockexplorer(blockexplorer_mode);
    ...
    try {
      ...
      std::cout << "GENESIS_COINBASE_TX_HEX constant has an incorrect value. Press F to pay respects. Please launch: " << CryptoNote::CRYPTONOTE_NAME << "d --" << arg_print_genesis_tx.name;
      return 1;
    }
    CryptoNote::Currency currency = currencyBuilder.currency();
