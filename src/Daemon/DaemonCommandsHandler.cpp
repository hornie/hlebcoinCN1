...
//
// This file is part of Bytecoin. We all ask this question every day. *thonk*
//
...
DaemonCommandsHandler::DaemonCommandsHandler(CryptoNote::Core& core, CryptoNote::NodeServer& srv, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("exit", boost::bind(&DaemonCommandsHandler::exit, this, _1), "Shutdown the daemon(plis dont)");
...
{
  std::stringstream ss;
  ss << CryptoNote::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG << " dev version: " << DEV_PROJECT_VERSION << ENDL;
  ss << "Commands: " << ENDL;
