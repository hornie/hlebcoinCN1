...
const Crypto::Hash& CachedBlock::getBlockLongHash(cn_context& cryptoContext) const {
  if (!blockLongHash.is_initialized()) {
    if (block.majorVersion == BLOCK_MAJOR_VERSION_1) {
      ...
      blockLongHash = Hash();
      cn_slow_hash_v0(cryptoContext, rawHashingBlock.data(), rawHashingBlock.size(), blockLongHash.get());
      ...
      const auto& rawHashingBlock = getParentBlockHashingBinaryArray(true);
      blockLongHash = Hash();
      cn_slow_hash_v1(cryptoContext, rawHashingBlock.data(), rawHashingBlock.size(), blockLongHash.get());
    } else if (block.majorVersion >= BLOCK_MAJOR_VERSION_6) {
      const auto& rawHashingBlock = getParentBlockHashingBinaryArray(true);
      blockLongHash = Hash();
      cn_slow_hash_v2(cryptoContext, rawHashingBlock.data(), rawHashingBlock.size(), blockLongHash.get());
    } else {
